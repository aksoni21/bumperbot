...
public:
    ...
    {
        declare_parameter<int>("simple_int_parameter", 28);
        declare_parameter<std::string>("simple_string_parameter", "Ankur");
...
private:
    ...
    rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter> & parameters)
    {
        ...
        for (const auto& param : parameters)
        {
            ...
            {
                RCLCPP_INFO_STREAM(get_logger(), "Received new parameter for simple_int_parameter: " << param.as_int());
                result.successful = true;
            }

            if (param.get_name() == "simple_string_param" && param.get_type() == rclcpp::ParameterType::PARAMETER_STRING)
            {
                RCLCPP_INFO_STREAM(get_logger(), "Received new parameter for simple_string_parameter: " << param.as_string());
                result.successful = true;
            }
        }
        return result;
